...
var ts_container = document.getElementById('root'); // find root tag

var ts_ajax = new XMLHttpRequest(); // ts_ajax 출력 결과 반환

var ts_content = document.createElement('div');
var TS_URL_ADDR = 'https://api.hnpwa.com/v0/news/1.json';
var TS_CONTENT_URL = 'https://api.hnpwa.com/v0/item/@id.json'; // 해당 콘텐츠의 url
...
function tsGetData(method, url, async) {
  if (method === void 0) {
    method = 'GET';
  }
...
function tsGetNewsFeed() {
  var newsFeed = ts_store.feeds; // json 데이터 객체 변환 후 리턴
  ...
  if (newsFeed.length === 0) {
    newsFeed = ts_store.feeds = tsMakeFeeds(tsGetData('GET', TS_URL_ADDR, false));
  }

  for (var i = (ts_store.currentPage - 1) * 10; i < ts_store.currentPage * 10; i++) {
